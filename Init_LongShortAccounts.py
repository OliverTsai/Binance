...
def get_top_long_short_accounts_2(symbol="BTCUSDT", startTimeStr = '',  limit = 500, period='5m', filename=""):
    startTime = date_to_ts(startTimeStr, "%Y-%m-%d %H:%M:%S")
    # endtime   = date_to_ts(endtimeStr, "%Y-%m-%d %H:%M:%S")

    request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
    result = request_client.get_top_long_short_accounts(symbol, period=period, startTime=startTime, limit=limit)
...
if __name__ == '__main__':
    get_top_long_short_accounts_2(startTimeStr=init_time(),period='1h', limit = 500,filename ="CSV/LongShortAccounts_1h.csv")
